...
struct BrushVertex_t
{
	...
	Math::CVector		m_Pos;
	Math::CVector		m_Normal;
	Math::CVector		m_TangentS;
	Math::CVector		m_TangentT;
	Math::CVector	m_TexCoord;
	Math::CVector2D	m_LightmapCoord;
...
{
	Math::CVector		m_vecVisOrigin;					// The point to to use as the viewpoint for area portal backface cull checks.
...
public:
	...
	virtual void ComputeTextureCoordinate(Math::CVector const& worldPos, Math::CVector2D& texCoord) = 0;
	virtual void ComputeLightmapCoordinate(Math::CVector const& worldPos, Math::CVector2D& lightmapCoord) = 0;
