...
struct ModelRenderInfo_t
{
	...
	const Math::matrix3x4_t* pModelToWorld;
	const Math::matrix3x4_t* pLightingOffset;
	const Math::CVector* pLightingOrigin;
	int flags;
	int entity_index;
	int skin;
	int body;
...
struct DrawModelState_t
{
	...
	const Math::matrix3x4_t* m_pModelToWorld;
...
struct DrawModelInfo_t
{
	...
	Math::CVector			m_vecAmbientCube[6];		// ambient, and lights that aren't in locallight[]
...
public:
	virtual int		DrawModel(int flags,
		...
		int entity_index,
		const model_t* model,
		Math::CVector const& origin,
		Math::QAngle const& angles,
		int skin,
		int body,
		int hitboxset,
		const Math::matrix3x4_t* modelToWorld = NULL,
		const Math::matrix3x4_t* pLightingOffset = NULL) = 0;
	...
	virtual void	SetViewTarget(const CStudioHdr* pStudioHdr, int nBodyIndex, const Math::CVector& target) = 0;
	...
	virtual void AddDecal(ModelInstanceHandle_t handle, Ray_t const& ray,
		Math::CVector const& decalUp, int decalIndex, int body, bool noPokeThru = false, int maxLODToDecal = ADDDECAL_TO_ALL_LODS) = 0;

	virtual void GMODAddDecal(ModelInstanceHandle_t handle, Math::CVector const&, Math::CVector const&, IMaterial* Material, int thing, float thing2, float thing3) = 0;
	...
	virtual Math::matrix3x4_t* DrawModelShadowSetup(IClientRenderable* pRenderable, int body, int skin, DrawModelInfo_t* pInfo, Math::matrix3x4_t* pCustomBoneToWorld = NULL) = 0;
	virtual void DrawModelShadow(IClientRenderable* pRenderable, const DrawModelInfo_t& info, Math::matrix3x4_t* pCustomBoneToWorld = NULL) = 0;
	...
	virtual bool DrawModelSetup(ModelRenderInfo_t& pInfo, DrawModelState_t* pState, Math::matrix3x4_t* pCustomBoneToWorld, Math::matrix3x4_t** ppBoneToWorldOut) = 0;
	virtual void DrawModelExecute(const DrawModelState_t& state, const ModelRenderInfo_t& pInfo, Math::matrix3x4_t* pCustomBoneToWorld = NULL) = 0;

	// Sets up lighting context for a point in space
	virtual void SetupLighting(const Math::CVector& vecCenter) = 0;
